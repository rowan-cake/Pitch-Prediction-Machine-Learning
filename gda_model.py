"""Per-pitcher GDA baseline with flattened pitch-history features."""
...
ARTIFACTS_DIR = Path("artifacts/gda")
DEFAULT_SEED = 440
DEFAULT_REG_PARAM = 0.1
DEFAULT_MAX_COMPONENTS = 16


def ensure_gda_dependencies() -> tuple[Any, Any, Any, Any]:
    try:
        import joblib
        import numpy as np
        import pandas as pd
        ...
        raise SystemExit(
            "Missing GDA dependencies. Install them with `python3 -m pip install -r requirements.txt` "
            "before running `train-gda`."
        ) from exc
    return np, pd, joblib, (PCA, QuadraticDiscriminantAnalysis, StandardScaler)
...
def train_gda_task(
    ...
    joblib: Any,
    PCA: Any,
    QuadraticDiscriminantAnalysis: Any,
    StandardScaler: Any,
) -> dict[str, Any]:
    ...
    class_counts = train_task_df[target_column].astype(str).value_counts()
    smallest_class_count = min(int(class_counts[label]) for label in label_names)
    max_components = min(
        args.max_components,
        x_train.shape[0] - 1,
        x_train.shape[1],
    ...
    if max_components < 1:
        return {
            "task_name": task_name,
            "status": "skipped",
            "skip_reason": "insufficient_samples_for_pca_gda",
            "retained_labels": retained_labels,
            "dropped_labels": dropped_labels,
            "row_counts": {
                "total": int(len(task_df)),
    ...
    model = QuadraticDiscriminantAnalysis(reg_param=args.reg_param)
    model.fit(x_train_pca, train_samples["labels"])

    train_predictions = model.predict(x_train_pca).tolist()
    test_predictions = model.predict(x_test_pca).tolist()
...
def summarize_gda_results(pitcher_summaries: list[dict[str, Any]], args: Any) -> dict[str, Any]:
    accuracies = []
    macro_f1s = []
    weighted_f1s = []
    skipped = []
...
def run_gda(args: Any) -> dict[str, Any]:
    np, pd, joblib, sklearn_objects = ensure_gda_dependencies()
    PCA, QuadraticDiscriminantAnalysis, StandardScaler = sklearn_objects
    ...
    for pitcher_number, (pitcher_name, pitcher_df) in enumerate(pitcher_groups, start=1):
        ...
        if not test_game_ids:
            ...
            pitch_type_metrics = train_gda_task(
                pitcher_name=pitcher_name,
    ...
    summary = summarize_gda_results(pitcher_summaries, args)
    write_summary(args.artifacts_dir / "summary.json", summary)
    print(f"GDA artifacts written to {args.artifacts_dir}")
    return summary
