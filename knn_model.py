"""Per-pitcher KNN baseline with flattened pitch-history features."""
...
def ensure_knn_dependencies() -> tuple[Any, Any, Any, Any]:
    try:
        import joblib
        import numpy as np
        import pandas as pd
        ...
        raise SystemExit(
            "Missing KNN dependencies. Install them with `python3 -m pip install -r requirements.txt` "
            "before running `train-knn`."
        ) from exc
    return np, pd, joblib, (KNeighborsClassifier, StandardScaler)
...
def skipped_task(reason: str, total_rows: int = 0) -> dict[str, Any]:
    return {
        "task_name": "location",
        "status": "skipped",
        "skip_reason": reason,
...
def train_knn_task(
    ...
    preprocessor["feature_count"] = preprocessor["context_size"] + args.sequence_length * preprocessor["history_feature_size"]
...
def build_pitcher_summary(
    ...
    return {
        ...
        "tasks": {
            ...
            "location": skipped_task("not_requested_pitch_type_focus"),
        },
    }


def summarize_knn_results(pitcher_summaries: list[dict[str, Any]], args: Any) -> dict[str, Any]:
    accuracies = []
    macro_f1s = []
    weighted_f1s = []
    skipped = []
...
def run_knn(args: Any) -> dict[str, Any]:
    np, pd, joblib, sklearn_objects = ensure_knn_dependencies()
    KNeighborsClassifier, StandardScaler = sklearn_objects
    ...
    for pitcher_number, (pitcher_name, pitcher_df) in enumerate(pitcher_groups, start=1):
        ...
        if not test_game_ids:
            ...
            pitch_type_metrics = train_knn_task(
                pitcher_name=pitcher_name,
    ...
    summary = summarize_knn_results(pitcher_summaries, args)
    write_summary(args.artifacts_dir / "summary.json", summary)
    print(f"KNN artifacts written to {args.artifacts_dir}")
    return summary
